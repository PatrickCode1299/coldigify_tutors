...
<div class="card bg-white p-4  all-modules-container">
	<div class="list-group">
		...
		<span onclick="changeVideo('<?php echo $modules->file1; ?>')" style="font-weight:bold; cursor:pointer;">{{$modules->course_title}}</span>
		<small style="clear:both; font-weight:bold;">{{$modules->module_title}}</small>
		</div>
		@endforeach
	</div>
</div>
<div class="video-container" style="flex:2; position:relative;">
	    <video  ontimeupdate="currentVideoTime()"  id="my-video" loading="lazy" style="width:100%; height:400px;">
  		<source id="video-display" src="/content/{{$video_details->file1}}" type="video/mp4">
  		Your browser does not support the video tag.
		</video>
		<div style="position: absolute; top:20px; left:50%; top: 20%;">
		...
		<div class="list-group-item p-4" style="margin-top: 10px;">
		<img alt="user-content" src="/storage/preview/{{$modules->preview}}" height="100px" width="100px" class="float-left img-rounded img-thumbnail">
		<span onclick="changeVideo('<?php echo $modules->file1; ?>')" style="font-weight:bolder; cursor:pointer;">{{$modules->course_title}}</span>
		<small style="clear:both; font-weight:bolder;">{{$modules->module_title}}</small>
		</div>
		 <?php
        $parameter=[
...
<script type="text/javascript">
	function changeVideo(source){
		...
		setCurrentVideo.setAttribute("src","/content/"+source);
